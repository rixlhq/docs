---
title: "Basic Usage"
description: "Learn how to implement the Video component with common patterns and use cases"
---

import {Tabs, TabsContent, TabsList, TabsTrigger} from "fumadocs-ui/components/tabs";
import {Callout} from "fumadocs-ui/components/callout";

## Importing the Video Component

```tsx
import {Video} from "@rixl/media-react";
```

## Essential Props

Every Video component needs at minimum:

```tsx
<Video
  id="your-video-id" // Required: Your video ID from dash.rixl.com/videos
/>
```

The `id` prop is mapped to `videoId` on the underlying `<rixl-video>` web component.

## Simple Video Player

```tsx
import {Video} from "@rixl/media-react";

function BasicVideo() {
  return (
    <div className="max-w-4xl mx-auto">
      <Video id="your-video-id" className="w-full h-auto rounded-lg" analyticsPage="standalone" />
    </div>
  );
}
```

## Listening to Analytics Events

```tsx
import {Video} from "@rixl/media-react";

function AnalyticsVideo() {
  return (
    <Video
      id="your-video-id"
      analyticsPage="standalone"
      onRixlAnalytics={(e) => {
        console.log("Analytics event:", e.detail.event);
      }}
    />
  );
}
```

## Lit / Vanilla HTML

```html
<script type="module">
  import "@rixl/media-lit/video";

  const video = document.querySelector("rixl-video");
  video.addEventListener("rixl-analytics", (e) => {
    console.log(e.detail.event);
  });
</script>

<rixl-video video-id="your-video-id" analytics-page="standalone"></rixl-video>
```

## Common Use Cases

### Full-Width Video

<Tabs defaultValue="hero">
  <TabsList>
    <TabsTrigger value="hero">Hero Background</TabsTrigger>
    <TabsTrigger value="contained">Contained Player</TabsTrigger>
  </TabsList>

  <TabsContent value="hero">
    ```tsx
    function HeroVideo() {
      return (
        <section className="relative h-screen overflow-hidden">
          <Video
            id="hero-video-id"
            autoPlay
            muted
            className="absolute inset-0 w-full h-full object-cover"
          />
          <div className="relative z-10 flex items-center justify-center h-full">
            <h1 className="text-4xl font-bold text-white">Welcome</h1>
          </div>
        </section>
      );
    }
    ```
  </TabsContent>

  <TabsContent value="contained">
    ```tsx
    function ContainedVideo() {
      return (
        <section className="max-w-4xl mx-auto py-12">
          <div className="aspect-video rounded-xl overflow-hidden">
            <Video
              id="video-id"
              className="w-full h-full"
            />
          </div>
        </section>
      );
    }
    ```
  </TabsContent>
</Tabs>

### Autoplay Preview

```tsx
function HoverPreview() {
  return (
    <div className="w-64 aspect-video">
      <Video id="preview-video-id" autoPlay muted className="w-full h-full object-cover" />
    </div>
  );
}
```

### Feed Video

```tsx
import {Feed} from "@rixl/media-react";

function FeedPage() {
  return <Feed feedId="my-feed" analyticsPage="feed" onRixlAnalytics={(e) => console.log(e.detail.event)} />;
}
```

## Best Practices

<Callout type="default">**Aspect Ratios**: Use CSS aspect-ratio classes to prevent layout shifts during video loading.</Callout>

<Callout type="info">
  **Autoplay**: The component mutes video by default to satisfy browser autoplay policies. You can also pass `muted={true}` explicitly.
</Callout>

<Callout type="warning">
  **Legacy props**: `theme`, `progressBar`, `allowFullscreen`, `heatmap`, `chapters`, and similar UI props are no longer forwarded. Use `className` and the browser `<video>` controls for now.
</Callout>
