...
```tsx
import "@rixl/media-react/style.css";
...
<Callout type="info">
  Need custom controls instead of the built-in player UI? Render them inside `<Video theme="hideUI">` or wrap `<Video>` and sibling controls in `<VideoPlayer>`, then read state with hooks like [`usePlayback`](/de/sdk/hooks/use-playback) and [`usePlayerProgress`](/de/sdk/hooks/use-player-progress) — they bind to the enclosing player scope automatically.
</Callout>
