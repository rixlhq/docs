...
<Callout type="info">
  The React `Image` component is a `@lit/react` wrapper around `<rixl-image>`. It accepts `id`, `className`, and analytics props (`analytics`, `analyticsPage`, `feedId`, `postId`, `onRixlAnalytics`). Legacy props such as `alt`, `loading`, `priority`, and `sizes` are accepted for backward compatibility but are not forwarded to the Lit element.
</Callout>
