...
<Callout type="info">
  The React `Video` component is a `@lit/react` wrapper around `<rixl-video>`. It forwards standard `<video>` attributes (`autoPlay`, `muted`, `loop`, `controls`, `className`) plus analytics props and the `onRixlAnalytics` event. Legacy UI props such as `progressBar`, `allowFullscreen`, `heatmap`, and `chapters` are no longer forwarded.
</Callout>
