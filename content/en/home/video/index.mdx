...
<Tabs items={['Basic Player', 'With Analytics', 'Lit / HTML']}>
...
<Callout type="info">
  The React `Video` component is a `@lit/react` wrapper around `<rixl-video>`. It forwards `id` (mapped to `videoId`), `src`, `videoData`, standard media attributes (`autoPlay`, `muted`, `loop`, `playsInline`, `poster`, `volume`), player UI props (`controls`, `theme`, `progressBar`, `showChapters`, `chapters`, `heatmap`, `hotSegments`, `allowFullscreen`, `allowPictureInPicture`, `autoHideMs`, `resumeProgress`, `soundDisabled`), analytics props, and the `onRixlAnalytics` event.
</Callout>
...
<Columns cols={3}>
  ...
  <Card title="React Integration" href="/en/sdk/media-react/video-component" icon="video">
...
<div className="mt-12">
  <Callout type="tip" title="Ready to stream?">
    ...
    Component Guide](/en/sdk/media-react/video-component) to add professional video players to your application. For custom workflows, explore our [API
