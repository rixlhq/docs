...
<Callout type="info">
  The React SDK now wraps the Lit web components (`<rixl-video>`, `<rixl-image>`, `<rixl-feed>`) with `@lit/react`. Use `onRixlAnalytics` to receive analytics events and standard media attributes (`controls`, `autoPlay`, `muted`, `loop`) on `Video`. Legacy UI props such as `progressBar`, `theme`, `showHeader`, and `enableInfiniteScroll` are no longer forwarded.
</Callout>
