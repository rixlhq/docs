...
The `id` prop is mapped to `imageId` on the underlying `<rixl-image>` web component. `className` is forwarded as expected and `alt` is accepted for backward compatibility but is not manually forwarded.
...
<Callout type="default">**Aspect Ratios**: Use CSS aspect ratio classes to prevent layout shifts during image loading.</Callout>
