...
When `true`, the feed restarts from the first post after the last post finishes playing.

```tsx
<Feed feedId="your-feed-id" loop={true} />
```
...
When `true`, the feed automatically advances to the next post when the current video ends or after a dwell timeout.

```tsx
<Feed feedId="your-feed-id" autoscroll={true} />
```
