...
function FeedPage() {
  return <Feed feedId="my-feed-id" analyticsPage="feed" limit={10} buffer={3} onRixlAnalytics={(e) => console.log(e.detail.event)} />;
}
```
...
- Support for `limit` and `buffer` pagination
- Automatic play/pause of visible video posts
- `autoscroll` and `loop` modes
- `feedId` propagation to all child posts
